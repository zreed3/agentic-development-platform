...
const agentContextScript = path.join(scriptDir, "agent-context.mjs");
...
function slice(args) {
  ...
  const raw = execFileSync(process.execPath, [agentContextScript, "feature", "--feature", feature, "--workflow", workflow, "--format", "json", "--no-manifest"], { cwd: root, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
...
if (args.command !== "slice") {
  console.log("Usage: node <adg-context.mjs> slice --feature S07 [--workflow agentic-tooling] [--format json|toon]");
